...
// Backend URL: ngrok public URL (changes if you restart ngrok without a reserved domain) or http://localhost:8080/api for local-only.
...
window.fetch = function (input, init) {
    ...
    const hitsBackend =
        url.startsWith(API_BASE) || url === root || url.startsWith(root + "/");
    if (!hitsBackend) {
    ...
    if (input instanceof Request) {
        return originalFetch.call(window, new Request(input, { ...init, headers }));
    }
    return originalFetch.call(window, input, { ...init, headers });
};

/** SockJS uses XMLHttpRequest for /info and xhr transports — not fetch — so ngrok's HTML interstitial can break the handshake unless this header is set on those XHRs. */
(function installSockJsNgrokXhrHeader() {
    if (!/ngrok/i.test(SERVER_ORIGIN)) return;
    const open = XMLHttpRequest.prototype.open;
    const send = XMLHttpRequest.prototype.send;
    XMLHttpRequest.prototype.open = function (method, url, ...rest) {
        this.__rafiqXhrUrl = url;
        return open.call(this, method, url, ...rest);
    };
    XMLHttpRequest.prototype.send = function (...args) {
        try {
            const raw = this.__rafiqXhrUrl;
            const abs = new URL(String(raw), document.baseURI).href;
            const root = getBackendOrigin();
...
/**
 * Single entry for STOMP over SockJS to /ws-office (room chat + notifications).
 * SockJS URL must stay path /ws-office — query strings can break SockJS routing on some servers.
 */
function createSockJsForOffice() {
    if (typeof SockJS === "undefined") return null;
    return new SockJS(`${SERVER_ORIGIN}/ws-office`);
}
...
// جزء التاسكات
// initApp() runs from the dark-mode DOMContentLoaded handler (and other entry points); async initApp loads rooms then starts refreshData on an interval.
...
function startNotificationRealtime() {
    ...
    const socket = createSockJsForOffice();
    if (!socket) {
...
{
    if (!provider) return;
    window.location.href = `${SERVER_ORIGIN}/oauth2/authorization/${encodeURIComponent(provider)}`;
}
